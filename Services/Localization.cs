...
public static class Localization
{
    ...
    private static readonly Dictionary<string, (string ru, string en)> _strings = new()
    {
        ...
        ["DeleteDbDesc"]       = ("Полностью удаляет library.db (треки, история, статистика, настройки) и перезапускает плеер. Файлы на диске не тронуты.", "Permanently deletes library.db (tracks, history, stats, settings) and restarts the player. Files on disk are untouched."),
        ...
        ["ConfirmDeleteDbMsg"] = ("library.db будет удалён вместе со всеми треками, историей, статистикой и настройками. Плеер перезапустится. Сами аудиофайлы на диске не пострадают.", "library.db will be deleted with all tracks, history, stats and settings. The player will restart. Your audio files on disk are not affected."),
